    ...
    def _reconnect(self, client):
        try:
            if self._client is not None:
                ...
                #callback
                if hasattr(self._client, "_callback_proxy") and self._client._callback_proxy is not None:
            ...
            while not self._terminated:
                ...
                if not self._terminated:
                    try:
                        ...
                        self._client._callback_proxy.ice_ping()
                    # except Ice.LocalException as ex:
                    except:
                        self._reconnect()
        except:
    ...
    while True:
        try:
            ...
            while not proc_queue.empty():
                try:
                    dev_list = proc_queue.get(timeout=2)
                except:
                    break
                if 0 < len(dev_list):
...
class DeviceCenterClient():
    ...
    def _set_callback(self):
        ...
        self._keep_callback_thread.name = "callback keep live"
    ...
    def _connect(self, timeout=5):
        ...
        while True:
            try:
                ...
                self._keep_live_thrd.name = "Session keep live"
    ...
    def _do_main_loop(self):
        ...
        while True:
            try:
                ...
                if len(self._whole_devices_list) == len(device_lists):
                    device_list_keys = [item.DeviceID for item in device_lists]
                    if 1 > len(list(set(device_list_keys) - set(self._whole_devices_list.keys()))):
                        time.sleep(60)
                        device_lists[:]= []
                        continue
